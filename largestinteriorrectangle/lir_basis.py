...
def _trim_grid(grid: np.array, center, target_ratio=.00):
    ...
    trimmed_grid[y_min:y_max + 1, x_min:x_max + 1] = grid[y_min:y_max + 1, x_min:x_max + 1]
    return trimmed_grid
...
def largest_interior_rectangle(grid: np.array, target_ratio=0.00, ratio_tolerance=.01, target_center_coord=None) -> np.array:
    rows, cols = grid.shape
    max_area = 0
    ...
    if target_center_coord:
        ...
        if 0 <= center_x < cols and 0 <= center_y < rows and grid[center_y, center_x] == 1:
          ...
          while (center_y + y_extend < rows and center_y - y_extend >= 0
              and center_x + x_extend < cols and center_x - x_extend >= 0
              and grid[center_y + y_extend, center_x] and grid[center_y - y_extend, center_x]
              and grid[center_y, center_x + x_extend] and grid[center_y, center_x - x_extend]):
                  y_extend += y_step
                  x_extend += x_step
                  if (center_y - y_extend >= 0 and center_y + y_extend < rows
                    and center_x - x_extend >= 0 and center_x + x_extend < cols
                    and grid[center_y - y_extend : center_y + y_extend + 1, center_x - x_extend : center_x + x_extend + 1].all()):
                        max_area = max(max_area, (x_extend * 2) * (y_extend * 2))
                        max_rect = (center_x - x_extend, center_y - y_extend, center_x + x_extend, center_y + y_extend)
        else:
      ...
      for y2 in range(rows):
          if (rows - y2) * cols < max_area:  # The remaining area is smaller than the max area
              break

          heights = (heights + 1) * grid[y2, :]  # compute the heights (histogram)
          stack = []  # reset stack at every row
          for x2 in range(cols + 1):
              if (rows - y2) * (cols + 1 - x2) < max_area:  # The remaining area is smaller than the max area
                  break
              ...
              while stack and (x2 == cols or heights[stack[-1]] > heights[x2]):
                  ...
                  if target_ratio * (1 - ratio_tolerance) <= curr_ratio <= target_ratio * (1 + ratio_tolerance) or target_ratio == 0:
                      area = width * height
